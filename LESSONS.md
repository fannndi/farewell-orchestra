...
| 2026-07-28 | MALFORMED_OUTPUT | Free profile performance analysis | tool_output max_bytes:38400 too small — frequent truncation of tool results causing agents to work with incomplete data. Compaction preserve_recent_tokens:5000 too aggressive — context loss between turns. | Conservative default settings not tuned for agentic workflow. | Bump tool_output to 51200 bytes, compaction preserve to 7000, reserved to 20000. |

*Append-only. Satu baris per insiden non-trivial. Jangan overwrite.*

| 2026-07-28 | Profile rework: drop paid+free, 4 hybrid variants | Config drift — paid too expensive for student budget | Budget constraint mismatch | Created 4 hybrid variants (hybrid-v1 to v4) with deepseek-v4-flash as primary orchestrator only, all sub-agents use free models. Updated opencode.jsonc, switch.bat, switch.sh. |
| 2026-07-28 | Final elimination: V1 winner after stress test | N/A | N/A | After heavy stress test across all 4 agent roles, V1 (deepseek-v4-flash + deepseek-free + north-mini-code) selected as winner. Profiles/, switch.bat, switch.sh deleted. Only opencode.jsonc kept. |