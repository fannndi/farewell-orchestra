...
| 2026-08-01 | executor regenerate config via cmd redirect → UnicodeDecodeError 0x97 di verify tool | FAIL → FIXED — area: executor + verify tool. Verify tool read_text utf-8 crash: output config pakai encoding locale cp1252, em dash U+2014 ditulis 0x97, bukan UTF-8 0xE2 0x80 0x94 | cmd /c python --stdout > file pakai encoding locale cp1252 di Windows | Fix: decode cp1252 → replace em dash → rewrite UTF-8 no-BOM. Lessons: output config wajib UTF-8 murni; PowerShell > menulis UTF-16 (rusak), pakai cmd redirect. |
| 2026-08-01 | parse error pre-existing di 3 hook .ps1 (dispatch, pre-generate, post-generate) memblok regenerasi config | FAIL → FIXED — area: PowerShell AST + executor. Em dash U+2014 di string/komentar PowerShell → brace/string tidak tertutup → parse error (missing closing }, missing catch/finally). Blocking hook beforeGenerate gagal, config example tidak ter-regenerate | Karakter Unicode em dash U+2014 tertanam di string/komentar PowerShell | Fix: scan byte non-ASCII (0xE2), ganti em dash → ASCII hyphen, validasi [Parser]::ParseFile errors.Count -eq 0 |
| 2026-08-01 | chunking reaktif → free model gagal | FAIL → FIXED — area: chunking protocol | Mekanisme lama 100% reaktif: free model terima task kegedean → CHUNK_REQUIRED/kosong → baru di-chunk. Researcher gagal >3 file (line 77), reviewer kosong 2x (line 79). Root cause: fan-out task besar utuh tanpa size estimation. | Fix: pre-chunk check proactive (hitung pertanyaan/file/format, 3+ trigger), unit 1-2 file/1 pertanyaan/≤8k token, max 3 chunk, sequential + CONTEXT_SUMMARY, CHUNK_DEPENDENCY_MAP + rollback, verify per chunk. ADR-002. |
